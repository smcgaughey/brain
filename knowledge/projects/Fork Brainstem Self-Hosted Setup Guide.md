...
Private GitHub Repo (knowledge files)
    │ git push
    ▼
GitHub App webhook → Your Cloudflare Worker
    │
    ├── Fetches changed files via GitHub API
    ├── Stores in Cloudflare R2 (object storage)
    ├── Indexes via Cloudflare AI Search (embeddings)
    └── Tracks installs in Cloudflare D1 (SQLite)
    
MCP Clients (Claude Desktop/Mobile/Web/Code)
    │
    └── Connect to Worker via MCP protocol
        ├── search_brain (semantic search)
        ├── get_document (fetch full file)
        ├── list_recent (recently modified)
        ├── list_folders (browse structure)
        ├── inbox (quick capture → commits to GitHub)
        └── about (server info)
```
...
2. Cloudflare account (free) — sign up at dash.cloudflare.com
...
- wrangler.toml — Cloudflare config (bindings, routes, resource IDs)
- src/ — TypeScript source code for the Worker
- site/ — Landing page assets
- CLAUDE.md — Dev guide (great for Claude Code)
...
# Outputs database_id — copy this for wrangler.toml
```

#### 2c. AI Search Instance
1. Cloudflare Dashboard → AI Search → Create
...
- account_id → your Cloudflare account ID (Dashboard → Overview → right sidebar)
- R2 bucket binding → your bucket name
- D1 database binding → your database_id
- AI Search binding → your instance name
...
1. In app settings → Private keys → Generate
2. Download .pem file
3. Convert format:
```bash
...
Note: Check the actual source code / CLAUDE.md for exact secret names — they may differ slightly from what's listed here.
...
3. Choose "Only select repositories" → select knowledge repo
4. Install → triggers initial sync
...
#### Claude.ai Web
Settings → Connectors → Add custom connector with URL and auth header
...
→ Brainstem commits directly to GitHub repo

## Learning Opportunities
1. Cloudflare Workers — serverless edge computing
2. Wrangler CLI — deployment tool
3. GitHub Apps — webhook-driven automation
4. MCP Protocol — how AI tools talk to external services
5. Vector embeddings — how semantic search works
6. R2/D1 — Cloudflare storage services
7. TypeScript — codebase language
8. OAuth — authentication flows

## Key Resources
- Source: https://github.com/dudgeon/git-brain
- TROUBLESHOOTING.md in repo
- CLAUDE.md in repo (dev guide — read this with Claude Code!)
- Cloudflare AI Search: https://developers.cloudflare.com/ai-search/
- Cloudflare Workers: https://developers.cloudflare.com/workers/
- Geoff Dudgeon (original author, personal contact)

## Migration Notes
- Basic Memory files are already .md — fully compatible
- Brainstem supports .md, .txt, .json, .yaml, .yml
- Current folder structure (health/, personal/, projects/, promotion/, research/) works as-is
- Run both Basic Memory and Brainstem in parallel during transition
- Basic Memory continues to work for claude.ai via current setup
- Brainstem adds: semantic search, mobile access, inbox capture
