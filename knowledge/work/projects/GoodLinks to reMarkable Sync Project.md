...
```
GoodLinks (tag: "rm") → Shortcut exports JSON → 
Python script → Fetch & convert to PDF → rmapi → reMarkable
```
...
├── config.yaml           # Settings (paths, tags, folders)
├── sync.py              # Main sync logic
├── converter.py         # URL → PDF conversion
├── remarkable.py        # rmapi wrapper
├── state.json           # Track what's been synced
...
- Triggers Shortcut → Python script chain
